...
**Plan 04's markdown page is on main** (2026-09-24, D136), replacing the interim page (D118).
The campaign page is at `/koa`; `/` redirects there temporarily. Markdown-style (pass two,
D139, below, supersedes the size, logo, and background here): literal `#` headings in bold
white, literal `---` dividers, and one call to action, the markdown link
`[Get early access to Koa](text Koa)` opening the `sms:` link to `SENDBLUE_PHONE_NUMBER`.
Sections: tagline, introduction, early access program (Layer 1 without a price, D127), who this
is for, frequently asked questions, and the footer "This page was intentionally generated by AI."
The copy is an agent draft traced to decisions in `content.ts`. **His copy review is the next
gate** (plus the tagline pick); the page stays not indexed until he
...
- **Page pass two shipped (D139, merged 2026-09-26):** #202020, a `name: altered` front-matter
  logo, #0080ff bold access link, 14px on pierre.computer's 5:3 line height with its 40px
  inset, Geist Mono trial behind one CSS variable (`--font-mono` in `globals.css`; plan 04's
  notes say how to flip back), the tagline as `**bold**` body text under the front matter, one
  h1 `# Introduction`, `##` sections, `###` questions, and a `> ` blockquote footer. Copy held
