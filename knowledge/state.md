...
The campaign page is at `/koa`; `/` redirects there temporarily. Markdown-style: one 15px size
for all text, literal `#` headings in bold white, literal `---` dividers, the text logo
`--- altered ---`, a #333333 dark background, and one call to action, the markdown link
`[Get early access to Koa](text Koa)` opening the `sms:` link to `SENDBLUE_PHONE_NUMBER`.
Sections: headline, subtitle, early access program (Layer 1 without a price, D127), who this is
for, frequently asked questions, and the footer "This page was intentionally generated by AI."
