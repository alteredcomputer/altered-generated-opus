...
**Status:** waiting. **Blockers:** Round 6 answers Q49 to Q52 (what the $12,500 buys, what arrives
at deposit, the buyer, the product date rule). Then a copy review round (Round 7) before indexing.
...
`generated.altered.computer` states the founding program exactly as locked: the tagline, what
ALTERED is in plain words, who it is for, what Layer 1 is, what the $12,500 buys, what arrives the
week of the deposit, the product clock, the price, the refund process, and the disclosure that
everything around the core is generated. Every fact on the page is read from one data file that
...
D015 pre-selling and the honest framing; D016 and D072 the core is hand-written, after sales;
D017 what the deposit buys (slot, price lock), to be re-read against Q50; D018 the buyer (Q51 may
widen); D019 outcome-led, never revenue; D020 the name; D022 proof is staged truth; D023 refund
through a process; D042 the page informs, the thread closes; D047 generated and disclosed; D052
two narratives; D060 distillation is not clarity; D065 the founding program, $1,000 deposit,
$12,500 anchor, three to eight seats; D067 the tagline; D071 the page is the single truth
artifact, article-shaped; D075 what ALTERED is.
...
1. Move the offer facts out of `apps/web/src/site/content.ts` into `knowledge/data/offer.json`
   ...
   process summary, launch or clock date if Q52 locks one.
2. Rewrite `content.ts` for the founding program. Article-shaped: it reads top to bottom as one
   ...
   is for and who it is not for; what Layer 1 is; what you get (Q49); what happens the week you
   reserve (Q50); the product clock (Q52); why we sell it before it exists (D015, D052); price
   and refund (D065, D023); frequently asked questions (seeded from the direction record's
   objections, grown from real ones); generated, and said so.
3. Call to action: keeps opening the text thread (D042). Wording per Q32 once answered; until
   then the current wording stays.
4. Remove the D040 launch-day deliverables section as a promise. If Q52 locks a relative clock,
   the mechanism list returns under that clock; if it drops the date, the list moves to "what the
   product is" with no date attached.
...
in the thread," until Round 6 lands. This is copy, so it ships only on his word.
...
1. `knowledge/data/offer.json` and its schema; build fails on a bad file; `content.ts` reads from
   it. Commit.
2. Rewrite the sections in the order above from the Round 6 answers. Every sentence that states a
